...
def Multiview_pcd_data(pipeline, align, T_base_camera, init_pose, save_dir, index, duration=1.0):
    ...
    while time.time() - start_time < duration:
        ...
        # 잘린 부분 확인
        # verts_invalid = verts[~valid]
        # pcd_valid = o3d.geometry.PointCloud()
        # pcd_valid.points = o3d.utility.Vector3dVector(verts_valid)
        # pcd_valid.paint_uniform_color([0, 1, 0])
        # pcd_invalid = o3d.geometry.PointCloud()
        # pcd_invalid.points = o3d.utility.Vector3dVector(verts_invalid)
        # pcd_invalid.paint_uniform_color([1, 0, 0]) 
        # coord = o3d.geometry.TriangleMesh.create_coordinate_frame(size=100, origin=[0, 0, 0])
        # o3d.visualization.draw_geometries([pcd_valid,pcd_invalid, coord])
...
def RANSAC(source_down, target_down, source_fpfh, target_fpfh):
    ...
    while attempt < max_attempts:
        ...
        for thr in thresholds:            
            ...
            if result.fitness > 0.90 and result.inlier_rmse < best_inlier_rmse:
